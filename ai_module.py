...
def _with_backoff(fn, max_retries=5):
    for attempt in range(max_retries):
        try:
            ...
            if getattr(e, "status_code", None) in (429, 500, 502, 503, 504):
                ...
                sleep_s = max(retry_after, 2**attempt) + random.uniform(0, 0.5)
                time.sleep(sleep_s)
                continue
            raise
    return fn()
...
def clean_turbine_data(
    filepath="wind_turbines.csv", save_cleaned=True, save_path="cleaned_data.csv"
):
...
def add_engineered_features(df, dropna: bool = True):
    ...
    df["capacity_zscore"] = (df["capacity_kw"] - df["capacity_kw"].mean()) / (
        df["capacity_kw"].std() if df["capacity_kw"].std() != 0 else 1.0
    )
...
def inject_synthetic_anomalies(
    ...
    anomalies["swept_area_m2"] = anomalies["rotor_diameter_m"] ** 2 * np.random.uniform(
        0.7, 0.9
    )
    ...
    df_combined = pd.concat([df, anomalies], ignore_index=True).sample(
...
def show_top_correlated_features(
    correlations: pd.Series, n: int = 10, use_streamlit: bool = False
):
...
def plot_top_feature_relationships(
    X: pd.DataFrame,
    scores: np.ndarray,
...
def plot_feature_correlation_heatmap(
    correlations: pd.Series, use_streamlit: bool = False
):
...
def prepare_input_data(
    ...
    project_number_turbines: int = None,
) -> pd.DataFrame:

    if training_mean is None or training_std is None:
        warnings.warn(
            "training_mean/training_std not provided — capacity_zscore set to 0. For best results, pass training stats.",
            UserWarning,
        )

    rotor_to_height_ratio = rotor_diameter / hub_height if hub_height != 0 else np.nan
    capacity_to_rotor_ratio = (
        capacity / rotor_diameter if rotor_diameter != 0 else np.nan
    )
    capacity_zscore = (
        ((capacity - training_mean) / training_std)
...
def predict_single_turbine(
    model: IsolationForest, turbine_df: pd.DataFrame, features: list = FEATURES
) -> int:
...
def summarize_prediction(
    turbine_df: pd.DataFrame, prediction: int, include_project_fields: bool = False
) -> str:
    ...
    if include_project_fields:
        if "project_capacity_kw" in turbine_df.columns:
            lines.append(
                f"- Project Capacity: {turbine_df['project_capacity_kw'][0]} kW"
            )
...
# Manual checks
def check_design_violations(
    capacity_kw, hub_height_m, rotor_diameter_m, swept_area_m2, total_height_m
):
    violations = []
    ...
    if total_height_m >= 152:
        violations.append(
            f"Total height is {total_height_m}m, which exceeds the 152m limit."
        )

    # Rule 2: Hub height < ~107 m
    if hub_height_m >= 107:
        violations.append(
            f"Hub height is {hub_height_m}m, which exceeds the recommended maximum of ~107m."
        )

    # Rule 3: Rotor diameter > 150 m as anomaly
    if rotor_diameter_m > 150:
        violations.append(
            f"Rotor diameter is {rotor_diameter_m}m, which is unusually large and may be anomalous."
        )

    # Rule 4: Check swept area consistency (πD²/4)
    expected_swept_area = (3.1416 * rotor_diameter_m**2) / 4
    if (
        abs(swept_area_m2 - expected_swept_area) > 0.1 * expected_swept_area
    ):  # >10% deviation
        violations.append(
    ...
    def _call():
        return client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You generate clear, expert-level summaries of anomaly detection results in wind turbine data.",
                },
